...
```bash
# Required
JWT_SECRET=your-secure-jwt-secret-for-overwatch
GOOGLE_CLIENT_ID=your-google-oauth-client-id.apps.googleusercontent.com

# Database root password (name matches config)
MYSQL_ROOT_PASSWORD=your-db-root-password

# Registry credentials (name matches config)
GHCR_TOKEN=ghp_your_github_token

# Optional: Initial admin emails (comma-separated)
ALLOWED_ADMIN_EMAILS=admin@example.com,dev@example.com
...
| `TENANTS_DIR` | From config | Override tenants directory path |
| `TEMPLATE_DIR` | `./tenant-template` | Override tenant template directory |
| `ALLOWED_ADMIN_EMAILS` | (none) | Comma-separated initial admin emails |
| `DATA_DIR` | `/app/data` | Directory for admin users file |
